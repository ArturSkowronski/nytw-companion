...
export function SuggestionsCard({ suggestions, fromPlan }: SuggestionsCardProps) {
  return (
    <div className="bg-[#0B0B0B] border border-[#1A1A1A] rounded-md p-5">
      ...
      {suggestions.length === 0 ? (
        <p className="text-sm text-[#9B9B9B]">No suggestions available.</p>
      ) : (
        <ul className="space-y-3">
          {suggestions.map((e) => (
            <li key={e.id}>
              <p className="font-mono text-sm text-[#F5F5F5]">{e.title}</p>
