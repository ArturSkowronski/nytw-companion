  ...
  return (
    <>
      ...
      <div className="fixed bottom-6 right-6 z-50 hidden sm:flex flex-col items-end gap-2">
        {open && (
          <div className="bg-[#0B0B0B] border border-[#262626] rounded-lg p-4 w-72 shadow-2xl">
            <p className="text-xs text-[#9B9B9B] font-mono mb-3">
              {items.length} event{items.length !== 1 ? 's' : ''} in your plan
            </p>
            <ul className="space-y-2 mb-3">
              {recentItems.map((item) => (
                <li key={item.event_id} className="text-xs text-[#F5F5F5] font-mono truncate">
                  · {item.event_id}
                </li>
              ))}
            </ul>
            <Link
              href="/my-plan"
              className="text-[#FF5B25] text-xs font-mono hover:underline"
        ...
        <button
          type="button"
          onClick={() => setOpen((o) => !o)}
          className="bg-[#FF5B25] hover:bg-[#e85a25] text-white font-mono font-bold text-sm px-4 py-2.5 rounded-full shadow-lg transition-colors flex items-center gap-2"
        >
      ...
      {/* Mobile: bottom bar */}
      <div className="fixed bottom-0 left-0 right-0 z-50 sm:hidden bg-[#0B0B0B] border-t border-[#262626]">
        <button
          type="button"
          onClick={() => setOpen((o) => !o)}
        ...
        >
          ...
          <span className="text-[#FF5B25] font-mono text-xs">
            {open ? 'close ✕' : 'view ›'}
          </span>
        </button>
        ...
        {open && (
          <div className="px-4 pb-4 border-t border-[#1A1A1A]">
            <ul className="space-y-2 py-3">
              {recentItems.map((item) => (
                <li key={item.event_id} className="text-xs text-[#9B9B9B] font-mono truncate">
                  · {item.event_id}
                </li>
              ))}
            </ul>
