      ...
      <section className="max-w-5xl mx-auto px-6 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-10">
          {sources.map((s) => (
            <article
              key={s.slug}
              className="rounded-lg border border-[#1A1A1A] bg-[#0B0B0B] p-5 flex flex-col gap-3"
            >
              <h2 className="font-mono text-lg font-bold text-[#F5F5F5]">{s.name}</h2>
              <p className="text-[#9B9B9B] text-sm leading-relaxed">{s.tagline}</p>
              <p className="font-mono text-xs">
                <span className="text-[#FF5B25]">Best for:</span>{' '}
                <span className="text-[#9B9B9B]">{s.bestFor}</span>
              </p>
              <a
                href={s.url}
                target="_blank"
                rel="noopener noreferrer"
                className="self-start mt-1 font-mono text-xs text-[#F5F5F5] border border-[#262626] rounded px-3 py-1.5 hover:bg-[#1A1A1A] hover:border-[#FF5B25] hover:text-[#FF5B25] transition-colors"
              >
                Visit →
              </a>
            </article>
