// Shared MCP tool definitions for the NYTW Engineer's Companion.
// Used by both /mcp (streamable HTTP) and /sse (legacy SSE) routes.
import { z } from 'zod'
...
export function registerCompanionTools(server: McpServer) {
  // -------------------- list_events --------------------
  server.registerTool(
    'list_events',
    {
      title: 'List events',
      description:
        'List the full 1,390-event NYTW 2026 catalogue. Optionally filter by day (YYYY-MM-DD, NYC time), tag slug, format, or host substring. Results are paginated.',
      inputSchema: {
  ...
  server.registerTool(
    'catalogue_stats',
    {
      ...
      description: 'Summary of the catalogue: counts by day, tag, format. Use this first to know what data is available.',
      inputSchema: {},
    },
    async () => {
...
export const COMPANION_SERVER_INFO = {
